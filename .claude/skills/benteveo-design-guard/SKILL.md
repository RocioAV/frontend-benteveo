...
description: >
  Guarda las restricciones de diseño del frontend Benteveo.
  El root de CSS es la fuente de verdad; las variables ORIGINALES no se modifican. Se autorizan
  tokens nuevos (concrete/error/motion/fuente Archivo) bajo revisión explícita.
  MOTION-FIRST: ninguna superficie puede quedar estática. Cualquier color/fuente NUEVO más allá
  de los tokens aprobados → requiere aprobación explícita.
...
- Cultura de herramientas honesta, marketplace de barrio (`docs/DESIGN.md`).
- Naranja Benteveo `#F2B705` = **ÚNICO acento quirúrgico**; neutros de taller (hormigón / papel / tinta).
- **Tipografía**: Archivo (variable) — reemplazó a Fredoka/Montserrat.
- **MOTION-FIRST**: springs en entradas, hovers y micro-feedback.
...
- **Tokens adicionales (ya en el root)**: `--color-concrete`, `--color-concrete-surface`,
  `--color-error`, `--duration-fast/base/slow`, `--ease-out-quint`, y fuente **Archivo** en
  `--font-title`/`--font-body`.
- Cualquier token NUEVO más allá de estos → **requiere aprobación explícita ANTES** de aplicarlo.

### 2. Color y fuente

- Usar SOLO tokens del root. Cero hex literales en componentes.
...
- **NUNCA** dejar una card o sección sin animación.
...
- **PERO el motion es prioridad**: un patrón animado de calidad (ej. "3 pasos" con
  conectores + ciclo activo) se respeta por encima de la limpieza anti-AI-tell.

### 5. Ante cualquier duda

- Cualquier duda de color, fuente o motion → **consultar ANTES de implementar. No asumir.**

## Resources

- **Dirección de diseño**: docs/DESIGN.md (tokens, tipografía, motion, higiene)
- **Plan de mejoras**: docs/PLAN-MEJORAS-UX-UI.md (sección 0 — restricciones)
