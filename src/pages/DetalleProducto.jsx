...
function DetalleProducto() {
  ...
  if (!product) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <Link to="/explorar" className="text-[var(--color-primary)] hover:text-[var(--color-amber-600)] font-semibold">
          Volver al catálogo
        </Link>
      </div>
    )
  ...
  const renderStars = (count) => {
    return Array.from({ length: 5 }, (_, i) => (
      <svg
        key={i}
        className={`w-4 h-4 ${i < count ? 'text-[var(--color-amber-400)]' : 'text-gray-300'}`}
        fill="currentColor"
        viewBox="0 0 20 20"
        aria-hidden="true"
      >
          ...
          <div>
            ...
            {reviews.length > 3 && (
              <button
                ...
                className="flex items-center gap-2 mt-4 text-sm font-semibold text-gray-900 underline underline-offset-4 hover:text-[var(--color-amber-600)] transition-colors"
              >
